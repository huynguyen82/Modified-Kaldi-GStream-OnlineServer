...
class DecoderPipeline2(object):
    def __init__(self, conf={}):
        ...
        self.cutter_threshold = conf.get("vad-threshold", 0.0)
    ...
    def create_pipeline(self, conf):
        ...
        ######> For bakup storing in wav format        
        ...
        if self.use_cutter:
            ...
            self.cutter.set_property("run-length",   100 * 1000000)
            self.cutter.set_property("threshold", self.cutter_threshold)
            self.asr.set_property("silent", True)
        ######> For converting and resampling
        caps = Gst.caps_from_string("audio/x-raw,format=S16LE,rate=16000,channels=1,layout=interleaved")
        self.fil = Gst.ElementFactory.make('capsfilter',"fil")
        self.fil.set_property("caps", caps)
        #####>
        ...
        ###>
        if self.use_cutter:
            for element in [self.appsrc, self.decodebin, self.audioconvert, self.audioresample, self.cutter, self.tee,
                            self.queue1, self.mp3enc, self.filesink,
        ...
        ## VAD
        if self.use_cutter: # add VAD into pipeline
        ...
        ###> For backup storing in wav format
        self.queue1.link(self.mp3enc)
        self.mp3enc.link(self.filesink)
        ###>
    ...
    def _on_partial_result(self, asr, hyp):
        logger.info("Adatation state: %s" % (len(self.get_adaptation_state().split())))        
    ...
    def init_request(self, id, caps_str):
        ...
        if self.outdir:
            ...
            self.filesink.set_state(Gst.State.NULL)
        ###> For backup storing in mp3 format
            self.filesink.set_property('location', "%s/%s.mp3" % (self.outdir, id))
        ###>
            self.filesink.set_state(Gst.State.PLAYING)
