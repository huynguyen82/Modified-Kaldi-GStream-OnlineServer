...
###>
import codecs # for backup storing
#from NLP_postprocessing.play_with_models import get_result_with_model #For NLP procesiing
###>
...
#	while self.state in [self.STATE_EOS_RECEIVED, self.STATE_CONNECTED, self.STATE_INITIALIZED, self.STATE_PROCESSING]:
    ...
    def received_message(self, m):
        ...
        if self.state == self.__class__.STATE_CONNECTED:
            ...
            self.content_id = props.get('content_id')
    ...
    def _on_full_result(self, full_result_json):
        try:
            ...
            full_result['user_id'] = self.user_id
            full_result['content_id'] = self.content_id
            full_result['content_type'] = self.content_type	
            if full_result.get("status", -1) == common.STATUS_SUCCESS:                
                ...
                try:
                    ...
                    e = sys.exc_info()[1]
                    logger.warning("Failed to send event to master: %s" % e)
            ###> For backup storing results in text file
		try:
                    if self.decoder_pipeline.outdir is not None:
                        fout = codecs.open(self.decoder_pipeline.outdir + '/' + self.request_id + ".txt", "aw", "utf-8")
                        strout = full_result["result"]["hypotheses"][0]["transcript"] + ' ' + str(full_result["result"]["hypotheses"][0]["confidence"]) + ' ' + str(full_result["segment-start"]) + ' ' + str(full_result["segment-length"]) + '\n'
                        fout.write(strout)
                        fout.close()
		except:
			e = sys.exc_info()[1]
			logger.warning("Failed to save result to disk: %s" % e)		
            ###>
    ...
    def send_adaptation_state(self):
        if hasattr(self.decoder_pipeline, 'get_adaptation_state'):
            ...
            #print("Adatation:",adaptation_state)
            event = dict(status=common.STATUS_SUCCESS,
    ...
    def post_process_full(self, full_result):
        if self.full_post_processor:
            logging.info("vao case full_post_process")
	...
	logging.info("full result: " + str(full_result))
...
def main():
	...
	logging.info("Calling API to AI platform")
