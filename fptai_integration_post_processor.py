...
def post_process_json(str_event):
    try:
        ...
        if "result" in event:
            ...
            if app_token and app_token != "none" and transcript != "<unknown>":
                logging.info("co vao su ly")
                if service_type == "qna":
                    result = get_qna(transcript, app_token)
                else:
                    intent = detect_intent(transcript, app_token)
            if intent is not None:
                event["intent"] = intent
            if result is not None:
                event["qna"] = result
            if confidence < 50 and len(transcript.split()) < 3:
		event["result"]["hypotheses"][0]["transcript"] = "<unknown>"
